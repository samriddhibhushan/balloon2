...
sound=loadSound("assets/uhuh.mp3");
birdS=loadSound("assets/bird.wav");
overS=loadSound("assets/over.wav");
}
...
//background image
bg = createSprite(displayWidth/2,485,1,1);
bg.addImage(bgImg);
bg.scale = 1.3


//creating top and bottom grounds
bottomGround = createSprite(displayWidth/2,displayHeight-210,displayWidth,20);
...
//creating balloon     
balloon = createSprite(400,displayHeight-350,20,50);
...
function spawnObstaclesTop() 
{
  if(World.frameCount % 60 === 0) {
    obstacleTop = createSprite(displayWidth,50,40,50);
...
{
      if(World.frameCount % 60 === 0) {
        obstacleBottom = createSprite(displayWidth,displayHeight-250,40,50);
...
function Score()
{
         if(balloon.isTouching(barGroup)&& gameState===PLAY)
         {
           score = score + 1;
         }
        textFont("algerian");
